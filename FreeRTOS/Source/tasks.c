...
signed portBASE_TYPE xTaskGenericCreate( pdTASK_CODE pxTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions )
{
	...
	signed char *newName;
 	...
 	if (strlen((char *)pcName) < 20) {
		newName = malloc(20);
		memcpy((char *)newName, (char *)"                   \0", 20);
	}
	else
	{
		newName = malloc(strlen((char *)pcName));
	}

 	memcpy((char *)newName, (char *)pcName, strlen((char *)pcName) );
	...
	if( pxNewTCB != NULL )
	{
		...
		prvInitialiseTCBVariables( pxNewTCB, newName, uxPriority, xRegions, usStackDepth );
